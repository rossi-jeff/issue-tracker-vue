<template>
  <div class="users">
    ...
    <b-card header="Users" border-variant="dark" header-bg-variant="light" class="m-2">
      <b-card-text>

        <b-button v-b-toggle.user-search size="sm" variant="outline-info" class="mx-2">
          <b-icon icon="search"/>
          Search
        </b-button>
        <b-button to="/users/new" size="sm" variant="outline-success" class="mx-2">
...
export default {
  ...
  methods: {
    async getUsers() {
      ...
      this.users = []
      let results = await this.api.getData(this.path, {}, buildHeaders({}))
      this.users = results
