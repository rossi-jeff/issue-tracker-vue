...
<div>
    ...
    <b-collapse id="nav-collapse" is-nav>
      <b-navbar-nav class="ms-auto">
        <b-nav-item-dropdown id="menu" text="menu" variant="info" class="m-md-2 mr-2">
          ...
          <b-dropdown-item to="/time_clock">Time Clock</b-dropdown-item>
          <b-dropdown-item v-if="session.signedIn" :to="{ name: 'Profile', params: { uuid: session.UUID } }">Profile</b-dropdown-item>
        </b-nav-item-dropdown>
  ...
  <b-modal id="login-modal" title="Login" header-bg-variant="primary" @ok="handleOk" ok-title="Login">
      ...
      if (confirm("Are you sure you wan to logout?")) {
        if (this.session.SessionId) {
          try {
            ...
            this.flash.error(`Error: ${e.message}`);
          }
        }
        await this.$store.dispatch("session/logout");
      }
